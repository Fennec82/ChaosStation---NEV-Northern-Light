	...
	if (prob(8))
		...
		if (areanames.len)
			switch(severity)
				...
				if (EVENT_LEVEL_MODERATE)
					command_announcement.Announce("Bioscans indicate that [chosen_mob] [chosen_verb] [english_list(areanames)]. Ironhammer is advised to approach with caution.", event_name, new_sound = 'sound/AI/vermin.ogg')
				if (EVENT_LEVEL_MAJOR)
